...
import (
	...
	internalmodels "github.com/codetaoist/laojun-plugins/internal/models"
	"github.com/codetaoist/laojun-plugins/internal/plugin"
...
func (s *ExtendedPluginService) deployInProcessPlugin(ctx context.Context, plugin *internalmodels.ExtendedPlugin) error {
	...
	case internalmodels.RuntimeGo:
		...
		_, err := s.inProcessManager.LoadPlugin(*plugin.BinaryPath, config)
		return err
...
func (s *ExtendedPluginService) deployMicroservicePlugin(ctx context.Context, plugin *internalmodels.ExtendedPlugin) error {
	...
	// 构建微服务插件配置
	microPlugin := &MicroservicePlugin{}
	microPlugin.ID = plugin.ID.String()
	microPlugin.Name = plugin.Name
	if plugin.DockerImage != nil {
		microPlugin.DockerImage = *plugin.DockerImage
	}
	if plugin.ServicePort != nil {
		microPlugin.ServicePort = *plugin.ServicePort
	}
	if plugin.HealthCheckPath != nil {
		microPlugin.HealthCheckPath = *plugin.HealthCheckPath
	}
	if plugin.GRPCProtoFile != nil {
		microPlugin.GRPCProtoFile = *plugin.GRPCProtoFile
	}
	if plugin.ServiceEndpoint != nil {
		microPlugin.ServiceEndpoint = *plugin.ServiceEndpoint
	}
	microPlugin.Status = string(plugin.Status)
	microPlugin.Config = make(map[string]interface{})
	microPlugin.CreatedAt = plugin.CreatedAt
	microPlugin.UpdatedAt = plugin.UpdatedAt

	return s.microserviceManager.DeployPlugin(microPlugin)
}
...
func (s *ExtendedPluginService) determinePluginType(pluginID string) (string, error) {
	// 首先尝试从进程内插件管理器获取插件实例
	if _, err := s.inProcessManager.GetPlugin(pluginID); err == nil {
		return "in_process", nil
	}

	// 然后尝试从微服务插件管理器获取插件实例
	if _, err := s.microserviceManager.GetPlugin(pluginID); err == nil {
...
func (s *ExtendedPluginService) callInProcessPlugin(ctx context.Context, request *plugin.GatewayRequest) (*plugin.PluginResult, error) {
	// 获取插件实例
	pluginInstance, err := s.inProcessManager.GetPlugin(request.PluginID)
	if err != nil {
	...
	case plugin.DataProcessor:
		return p.ProcessData(ctx, request.Params)
	case plugin.ImageFilter:
...
// callMicroservicePlugin 调用微服务插件实例
func (s *ExtendedPluginService) callMicroservicePlugin(ctx context.Context, request *plugin.GatewayRequest) (*plugin.PluginResult, error) {
	return s.microserviceManager.CallPlugin(request.PluginID, request.Method, request.Params)
}
