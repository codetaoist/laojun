...
type FileConfig struct {
	Filename   string `yaml:"filename" env:"LOG_FILE" config:"log.file.filename" default:"./logs/app.log"`
...
func New(config Config) Logger {
	...
	case "file":
		// 确保日志目录存在
		if err := os.MkdirAll(filepath.Dir(config.File.Filename), 0755); err != nil {
...
func (l *loggerImpl) WithError(err error) Logger {
	...
	// 添加错误堆栈信息
	if err != nil {
		entry = entry.WithField("error_type", fmt.Sprintf("%T", err))
