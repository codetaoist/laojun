...
func (s *PermissionService) CheckUserPermission(req models.UserPermissionCheckRequest) (*models.UserPermissionCheckResponse, error) {
	// 首先尝试从缓存获取结果
	if s.cache != nil {
		cachedResult, err := s.cache.GetUserPermission(req.UserID, req.DeviceType, req.Module, req.Resource, req.Action)
		if err == nil && cachedResult != nil {
			return &models.UserPermissionCheckResponse{
				HasPermission: cachedResult.HasPermission,
				Reason:        cachedResult.Reason + " (缓存)",
			}, nil
		}
	}
	...
	if s.cache != nil {
		...
		s.cache.SetUserPermission(req.UserID, req.DeviceType, req.Module, req.Resource, req.Action,
			result.HasPermission, result.Reason, expiry)
	}
